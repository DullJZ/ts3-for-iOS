...
| iOS accessibility and compact layout | Partial | SwiftUI shared sheets/forms, compact voice status strip, VoiceOver labels/values for global mic/output/transmit/whisper state, and readable row summaries/actions for dense permission, group, complaint, ban, privilege-key, contact, poke, file-browser, file-transfer, server-log, and offline-message administration lists | Continue auditing dynamic type and remaining dense administration sheets. |
