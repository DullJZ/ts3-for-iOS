...
| Audio devices and profiles | Partial | Audio routes, route availability notes, input device row copy summaries with VoiceOver actions, audio settings import/export with sanitized import preview, profile import/export with merge/sanitize tests, profile import preview with new/replaced/skipped/adjusted counts and copyable summary, profile and per-user playback row copy summaries with VoiceOver actions, user playback backup import/export with sanitized preview | Validate route switching behavior on real iOS and Mac Catalyst devices. |
