...
| Channel create/edit/delete/move | Partial | Channel editor with codec quality/latency/encryption controls, voice/music codec presets, needed talk/subscribe/description-view powers, create/edit channel sort order, codec configuration summaries, legacy/encryption codec diagnostics, official range validation, move preview, draft import/export, icon import, password prompts, and permission inheritance impact visibility | Validate advanced codec controls against real servers and add deeper inherited effective-value tracing if protocol data allows it. |
