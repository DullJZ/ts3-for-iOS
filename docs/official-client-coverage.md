...
| Client actions | Partial | Client row menus, centralized talk request queue with grant/deny/copy and batch handling, online user complaint/contact shortcuts, database-record and group-member contact/note plus online poke/private-message actions, database-record and group-member offline-message/complaint/UID-ban context actions, database-client row copy summaries with VoiceOver actions, auditable client-database backup import with selectable restore preview, database filter preset backup import/export with selectable restore preview, field completeness summaries, and copyable summaries, move/kick/ban/message style workflows in UI | Audit all official context-menu actions and fill remaining bookmark-contact edge actions. |
